...
#last layers only , so 15 and 32
layers="15 32"  # Using seq to create a range from 0 to 32 (for layers 0 to 32)
...
#last modification to process as attention mat crashed
layers="32"
models_s=( "esm2_t33_650M_UR50D" )
chains=( "heavy_chain" )
complexities=( "attention_matrices_average_layers"  )
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "ALL_CDRH" ]]; then
    ...
    input_embeddings="/scratch/niccoloc/tz_heavy/${model}/${complexity}/tz_heavy_${model}_${complexity}_layer_$((layer+1)).npy"
  elif [[ "$chain" == "cdr3_only" ]]; then
    input_idx="/doctorai/userdata/airr_atlas/data/embeddings/trastuzumab_npy/${model}/tz_cdr3_100k_idx.csv"
    # input_idx="/scratch/niccoloc/tz_heavy/${model}/tz_heavy_chain_100k_idx.csv"
    input_embeddings="/doctorai/userdata/airr_atlas/data/embeddings/trastuzumab_npy/${model}/${complexity}/tz_cdr3_100k_${model}_${complexity}_layer_$((layer+1)).npy"
  fi
}
      ...
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="/doctorai/niccoloc/Vicinity_results_100k_EUCLIDIAN"
          else
            result_dir="/doctorai/niccoloc/Vicinity_results_100k_EUCLIDIAN_sample${sample_size}"
          fi
          ...
          command=(
            "python" "-u" "/doctorai/niccoloc/airr_atlas/quantitative_mapping_github/scripts/Vicinity_pipeline_final.py"
