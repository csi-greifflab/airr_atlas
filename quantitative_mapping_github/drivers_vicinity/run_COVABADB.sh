...
input_metadata="../data/sequences/antigens/covabdab_AND_background.tsv"
LD_matrix= '../data/results/LD_covabdab_bg2.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    ...
    input_idx="../data/embeddings/covabdab/${model}/covabdab_bg_ESM2_idx.csv"
    # input_idx="../data/embeddings/covabdab/${model}/covabdab_bg_AB2_idx.csv"
    input_embeddings="../data/embeddings/covabdab/${model}/${complexity}/covabdab_${model}_${complexity}_layer_$((layer+1)).npy"
                       
  else
    #ignored as we only use paired chain in covabdab experiments
    input_idx="../data/embeddings/brian_hie/cr9114_hie_idx.csv"
    input_embeddings="../data/embeddings/brian_hie/${model}/${complexity}/brian_hie_${model}_${complexity}_layer_$((layer+1)).npy"
  fi
}
    ...
    for model in "${models_s[@]}"; do
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          # # Adjust max_parallel_jobs based on chain and complexity --- could be necessary based on resources available
        ...
        #   if [[ "$complexity" == "cdr3_extracted_unpooled" || "$complexity" == "embeddings_unpooled" && "$chain" == "heavy_chain" ]]; then
        #   max_parallel_jobs=5
        #   elif [[ "$chain" == "paired_chain" && "$complexity" != "embeddings" ]]; then
        #   max_parallel_jobs=3
        #   else
        #   max_parallel_jobs=7
        #   fi
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../data/results/Vicinity_results_COVABDAB_Density2"
          else
            result_dir="../data/results/Vicinity_results_COVABDAB_Density2_sample${sample_size}"
          fi
