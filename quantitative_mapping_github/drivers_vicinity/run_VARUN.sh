...
input_metadata="../data/sequences/brian_hie/cr9114_hie_metadata.csv"
LD_matrix='../data/results/LD_mat_brian_hie.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    #ignored as we only use heavy_chain in varun (brian hie) experiments
    input_idx="../data/embeddings/brian_hie/${model}/brian_hie${chain}_${model}_idx.csv"
    input_embeddings="../data/embeddings/brian_hie/${model}/${complexity}/brian_hie_${model}_${model}_${complexity}_layer_$((layer+1)).npy"
  else
      ...
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../data/results/Vicinity_results_BRIANHIE_Density2"
          else
            result_dir="../data/results/Vicinity_results_BRIANHIE_Density2_sample${sample_size}"
          fi
