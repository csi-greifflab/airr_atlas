...
input_metadata="../data/sequences/porebski/porebski_metadata_vsALL.csv"
LD_matrix='../data/results/porebski_LD_matrix.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    ...
    input_idx="../data/embeddings/porebski_npy/${model}/porebski_${chain}_${model}_idx.csv"
    input_embeddings="../data/embeddings/porebski_npy/${model}/${complexity}/porebski_${chain}_${model}_${model}_${complexity}_layer_$((layer+1)).npy"
  else
    input_idx="../data/embeddings/porebski_npy/${model}/porebski_${chain}_idx.csv"
    input_embeddings="../data/embeddings/porebski_npy/${model}/${complexity}/porebski_${chain}_${model}_${complexity}_layer_$((layer+1)).npy"
  fi
}
      ...
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../data/results/Vicinity_results_POREBSKI_Density2"
          else
            result_dir="../data/results/Vicinity_results_POREBSKI_Density2_sample${sample_size}"
          fi
