...
input_metadata="../data/sequences/antigens/alphaseq_sars/metadata_alphaseq_HB_LB.csv"
LD_matrix='../data/results/LD_alphaseq_HB_LB.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    input_idx="../data/embeddings/alphaseq44/${model}/alphaseq_paired_chain_${model}_idx.csv"
    input_embeddings="../data/embeddings/alphaseq44/${model}/${complexity}/alphaseq_${model}_${complexity}_layer_$((layer+1)).npy"
  else
    #ignored as we only use paired chain in alphaseq experiments
      ...
      for complexity in "${complexities[@]}"; do
        ...
        #   fi
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../data/results/Vicinity_results_ALPHASEQ_Density2_HB_LB"
          else
            result_dir="../data/results/Vicinity_results_ALPHASEQ_Density2_sample${sample_size}"
          fi
