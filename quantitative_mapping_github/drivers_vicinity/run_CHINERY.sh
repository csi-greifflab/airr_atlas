...
LD_matrix="../data/results/tz_LD_dist_mat_HB_LB.npy"

# Construct folder name
if [[ $sample_size == 0 ]]; then
  result_dir="../data/results/Vicinity_results_100k_4"
else
  result_dir="../data/results/Vicinity_results_100k_sample${sample_size}"
fi
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "ALL_CDRH" ]]; then
    input_idx="../data/embeddings/trastuzumab_npy/${model}/tz_${chain}_100k_${model}_idx.csv"
    input_embeddings="../data/embeddings/trastuzumab_npy/${model}/${complexity}/tz_${chain}_${model}_${complexity}_layer_$((layer+1)).npy"
  else
    input_idx="../data/embeddings/trastuzumab_npy/${model}/tz_${chain}_100k_idx.csv"
    # input_idx="../data/embeddings/tz_heavy/${model}/tz_heavy_chain_100k_idx.csv"
    input_embeddings="../data/embeddings/tz_heavy_v2/${model}/${complexity}/tz_${chain}_${model}_${complexity}_layer_$((layer+1)).npy"
  fi
}
    ...
    for model in "${models_s[@]}"; do
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          # Construct folder name
          if [[ $sample_size == 0 ]]; then
            result_dir="../data/results/Vicinity_results_100k_Density2"
          else
            result_dir="../data/results/Vicinity_results_100k_Density2_sample${sample_size}"
          fi
