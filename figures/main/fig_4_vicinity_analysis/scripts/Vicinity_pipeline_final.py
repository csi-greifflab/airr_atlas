...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="junction_aa",
        help="Column name for junction aa",
    )
    parser.add_argument(
        "--df_affinity_colname",
        type=str,
        default="affinity",
        help="Column name for affinity",
    )
    parser.add_argument(
        "--input_idx",
        type=str,
        default="",
        help="File containing the corrispondence between the embeddings index and the metadata id",
    )
    parser.add_argument(
        "--input_metadata",
        type=str,
    ...
    parser.add_argument(
        "--result_dir",
        type=str,
        default="./Vicinity_results",
        help="parent directory of results",
    )
    parser.add_argument(
        "--save_results", action="store_true", help="Flag to save results"
    )
    parser.add_argument("--compute_LD", action="store_true", help="Flag to compute LD")
    parser.add_argument(
        "--plot_results", action="store_true", help="Flag to generate plots"
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
        help="parallelize KNN search , suggested to use with more than 500k seqs",
    )
    parser.add_argument(
        "--chosen_metric",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Size of the max sample of each label",
    )
    parser.add_argument(
        "--LD_sample_size",
        type=int,
        default=10000,
        help="Number of seqs (X vs ALL) to check in the LD calculations",
    )
    parser.add_argument(
        "--precomputed_LD",
        type=str,
        required=False,
        help="path of the precomputed file.csv with LD results",
        default="",
    )
    parser.add_argument(
        "--radius_range",
        type=str,
        default="7,24,1",
        help="Specify the min and max radius and steps separated by a comma (e.g., '7,24,1')",
    )
    parser.add_argument(
        "--skip_knn", action="store_true", help="Flag to skip the  KNN Vicinity "
    )
    parser.add_argument(
        "--LD_matrix",
        type=str,
        required=False,
        default="",
        help="path of the precomputed file.pt with LD results",
    )
    parser.add_argument(
        "--fix_nan_inf",
        action="store_true",
        default="True",
        help="Fix NaN and inf values in the embeddings, by replacing them with 0",
    )
...
def create_result_folder(res_folder):
    if not os.path.exists(res_folder):
        ...
        print(
            f"Warning: Result folder {res_folder} already exists. Output files may be overwritten."
        )
...
def load_data(input_metadata, input_embeddings, idx_reference):
    ...
    if seqs.isna().any().any():
        ...
        if (
            args.df_affinity_colname in seqs.columns
            and seqs[args.df_affinity_colname].isna().any()
        ):
            print(
                f"Found {seqs[args.df_affinity_colname].isna().sum()} NaN values in the binding label column '{args.df_affinity_colname}'. Converting them to 'unkn'..."
            )
            seqs[args.df_affinity_colname] = seqs[args.df_affinity_colname].fillna(
                "unkn"
            )
    ...
    if idx_reference == "":
        ...
        tensors_df = pd.DataFrame(
            {"id": np.arange(0, len(tensors)), "embedding": list(tensors)}
        )
    ...
    print(
        f"Number of sequences in the dataset after de-deuplication: {len(df)}, per class: {df[args.df_affinity_colname].value_counts()}"
    )
...
def filter_data(
    ...
    for label in unique_labels:
        ...
        filtered_df = df[df[affinity_col] == label].sample(
            n=curr_sample_size, random_state=rand_seed
        )
...
def sample_affinities(df, sample_size, df_affinity_colname="affinity"):
    ...
    for affinity, count in count_per_affinity.items():
        ...
        sampled_indices = sampled_indices.append(
            df[df["affinity"] == affinity]
            .sample(n=current_sample_size, random_state=42)
            .index
        )
...
def adjacency_to_csv(matrix, filename):
    ...
    df = pd.DataFrame({"row": id_index_sample, "Neighbors_Count": neighbors_counts})
...
def check_and_fix_embeddings(embeddings, fix_nan_inf=True):
    ...
    if np.isinf(embeddings).any():
        ...
        if inf_count > 0:
            ...
            print(
                f"Found {inf_count} inf values in the embeddings at indices (row, col): {inf_indices[:10]},\nunique columns: {unique_cols}"
            )
            if inf_count > 10:
        ...
        if fix_nan_inf:
            ...
            print(
                "ERROR! Infinite values found in the embeddings. Replacing them with 0."
            )
        else:
            raise ValueError(
                "Embeddings contain infinite values. Please check your input data."
            )
    ...
    if np.isnan(embeddings).any():
        ...
        if nan_count > 0:
            ...
            print(
                f"Found {nan_count} NaN values in the embeddings at indices (row, col): {nan_indices[:10]},\nunique columns: {unique_cols}"
            )
            if nan_count > 10:
        ...
        if fix_nan_inf:
            ...
            raise ValueError(
                "Embeddings contain NaN values. Please check your input data."
            )
    return embeddings
...
try:
    min_radius, max_radius, step = (
        float(args.radius_range.split(",")[0]),
        float(args.radius_range.split(",")[1]),
        float(args.radius_range.split(",")[2]),
    )
    ...
    print(
        "Error: Please ensure you provide two integers separated by a comma for the radius range."
    )
...
id_index_sample_df = id_index_sample.reset_index().rename(
    columns={"index": "vicinity_index"}
)
matching_indices_df = id_index_sample_df.merge(
    df[["id", "sequence_id"]], on="id", how="left"
)
matching_indices_df.columns = ["vicinity_index", "metadata_id", "sequence_id"]
matching_indices_df.to_csv(
    f"{result_folder}_index_mapping_{analysis_name}.csv", index=False
)
...
# KNN vicinity
vicinity_analysis_instance = Vicinity_analysis(
    df,
    embeddings,
    neighbor_numbers,
    id_index_sample,
...
# Run Vicinity Radius
percentages_results, perc_df, res_df, mean_num_points, LD1_res, LD2_res = (
    vicinity_analysis_instance.perc_cosine_radius(ED_radius)
)
...
density_thresholds = (
    density_thresholds + lin0_density_thr + LD_thr
)  # choose the lin0 - the first threshold
print(f"Computing adjancey for the following density thresholds: {density_thresholds}")
adjacency_matrices, row_ids = vicinity_analysis_instance.compute_adjacency_matrices(
    density_thresholds
)
...
for i, matrix in enumerate(adjacency_matrices):
    ...
    if i + 1 < len(density_thresholds) and reformat_density == format_density(
        density_thresholds[i + 1], precision=4
    ):
    ...
    if i < len(knn_density_thr):
        ...
        adjacency_to_csv(
            matrix,
            f"{result_folder}neighbors_diag_NN{nn_info}_R{reformat_density}_{analysis_name}.csv",
        )
        ...
        adjacency_to_csv(
            matrix,
            f"{result_folder}neighbors_diag_lin{lin_info}_R{reformat_density}_{analysis_name}.csv",
        )
        save_npz(matrix_name, matrix)
    else:
        ...
        adjacency_to_csv(
            matrix,
            f"{result_folder}neighbors_diag_LD{ld_info+1}_R{reformat_density}_{analysis_name}.csv",
        )
        save_npz(matrix_name, matrix)
...
pd.merge(
    vicinity_analysis_instance.df[["id", "sequence_id"]],
    pd.DataFrame({"id": id_index_sample.reset_index(drop=True)}),
    on="id",
    how="inner",
).to_csv(f"{result_folder}index_sequence_id_{analysis_name}.csv", index=False)

# adjacency_matrices[2] is the LD-1 threshold
adjacency_to_edgelist(
    adjacency_matrices[2],
    matching_indices_df,
    output_file=f"{result_folder}edges_list_avgLD1_{analysis_name}.csv",
)  # adjacency_matrices[2] is the LD-1 threshold


# ----------------- Save vicinity results Pickle ------
if args.save_results:
    vicinity_analysis_instance.save_to_pickle(
        f"{result_folder}Vicinity_{analysis_name}.pkl"
    )
...
LD_filename = (
    f"{result_folder}d_mean1_summary_LD_{analysis_name}_{args.sample_size}k.csv"
)

if args.LD_matrix != "":
    print(
        "Using the LD matrix file at ",
        args.LD_matrix,
        " by the  get_LD_matrix.py script ",
    )
    ...
    d_res1_long.to_csv(
        f"{result_folder}d_whole_LD_stats_{analysis_name}_{(len(id_index_sample))/1000}k.csv"
    )
