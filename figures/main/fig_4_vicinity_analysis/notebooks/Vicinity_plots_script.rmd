...
```{r}
# Load pre-computed figures if they exist, or use the summarized CSVs below
# obj_name <- load("../data/results/Vicinity_main_fig.rds")
# v1 <- get(obj_name)
# print(v1)
```
...
# Define the directory containing the files (relative to script location)
directory <- c("../data/results/vicinity_raw_extracted") # This would be where the outputs of the Vicinity_pipeline_final.py script are stored
...
# fwrite("../data/results/Vicinity_results_TZ_ALL.csv", Pbox_data)

# LOADING PRE-COMPUTED DATA FOR GITHUB REPO REPRODUCIBILITY
if (!exists("Pbox_data") && file.exists("../data/results/Vicinity_results_MAIN.csv")) {
  Pbox_data <- fread("../data/results/Vicinity_results_MAIN.csv")
}
...
for (chosen_LD_density in LD_densities) {  
  ...
  dir = '../data/results/LD_score_500' # Example path for LD results
  files <- list.files(dir, full.names = TRUE, recursive = TRUE)
...
# fwrite("../data/results/Vicinity_results_LD_plot.csv", LD_plot)

# LOADING PRE-COMPUTED LD DATA FOR GITHUB REPO REPRODUCIBILITY
if (!exists("LD_plot") && file.exists("../data/results/Vicinity_results_LD_plot.csv")) {
  LD_plot <- fread("../data/results/Vicinity_results_LD_plot.csv")
}
...
ggsave("../output/fig_4c_vicinity_density_prelim.png", last_plot(), width =10, height =6.25, units = "in", dpi = 300)

save(p4_1, file= "../data/results/Vicinity_main_fig.rds")
...
ggsave("../../supplementary/fig_s8/output/fig_s8_tz_supplementary_layers.png", last_plot(),  width =10, height =6.25, units = "in", dpi = 300)
save(P3, file= "../data/results/TZ_supplementary_layers.rds")
...
ggsave("../../supplementary/fig_s10/output/fig_s10_dms_supplementary_layers.png", last_plot(), width =10, height =6.25, units = "in", dpi = 300)
save(P4, file= "../data/results/DMS_supplementary_layers.rds")
...
#paste the ../ before the directory name
...
fwrite("../Vicinity_results_SAMPLE.csv", Pbox_data_SAMPLE)
...
save(p2, file= "../Vicinity_SAMPLE_SIZE2.rds")
download_rstudio_file(
...
ggsave("../figures/airr_atlas/Vicinity_SAMPLE_SIZe_prelim.png", last_plot(), width =10, height =6.25, units = "in", dpi = 300)
...
sample_LD= c(
  '../Vicinity_results_100k_Density2_sample5000_LD_ONLY/Pooled_antiberta2-cssp_cdr3_embeddings_sample_5000_layer_0_LD_ONLY',
  '../Vicinity_results_100k_Density2_sample25000_LD_ONLY/Pooled_antiberta2-cssp_cdr3_embeddings_sample_25000_layer_0_LD_ONLY',
  '../Vicinity_results_100k_Density2_sample2500_LD_ONLY/Pooled_antiberta2-cssp_cdr3_embeddings_sample_2500_layer_0_LD_ONLY',
  '../Vicinity_results_100k_Density2_sample10000_LD_ONLY/Pooled_antiberta2-cssp_cdr3_embeddings_sample_10000_layer_0_LD_ONLY'
)
...
# for (chosen_LD_density in LD_densities) {  
  ...
  # dir = '../Vicinity_results_sample_test/LD_score_500'
...
# Define the directory containing the files
directory <-c ( "../Vicinity_results_BRIANHIE_Density2",
                "../Vicinity_results_POREBSKI_Density2",
                "../Vicinity_results_COVABDAB_Density2",
                "../Vicinity_results_ALPHASEQ_Density2_HB_LB"
                
                )



# directory <- "../Vicinity_results_sample_test/LD_score_500"
...
results_list <- foreach(dir = dirs_vicinity,
                    ...
                    # dir ="../Vicinity_results_ALPHASEQ_Density2/Pooled_esm2_t33_650M_UR50D_paired_chain_layer_8" 
    dataset <- case_when(
...
fwrite("../Vicinity_results_DMS.csv", Pbox_data_DMS)
...
DMS_p1 = Pbox_data_DMS %>% 
  inner_join(best_median_DMS %>% 
         ...
         mutate(layer= as.numeric(layer),
                DMS = factor( DMS, levels = c( "porebski","brian_hie","alphaseq", "covabdab")),
                IQR= Q3 - Q1)
fwrite(DMS_p1,"../Vicinity_results_DMS_p1.csv")
...
ggsave("../figures/airr_atlas/Vicinity_DENSITY_DMS_prelim.png", last_plot(), width =10, height =6.25, units = "in", dpi = 300)
ggsave("../figures/airr_atlas/Vicinity_DENSITY_DMS_prelim_2.png", last_plot(), width =7.5, height =4.69, units = "in", dpi = 300)
...
```{r Final Aesthetic Adjustments to all plots}
library(patchwork)
library(cowplot)
library(grid)
library(gtable)

# Map existing variables to the ones used in the new snippet
v1 <- p4_1
s1 <- p2
dms1 <- p3

# Load additional pre-computed objects if they exist
# Note: These are generated by tsne_density_plot_script.Rmd
if (file.exists("../data/results/density_DIFF_plot.rds")) {
  obj_name <- load("../data/results/density_DIFF_plot.rds")
  density_main_fig <- get(obj_name)
}
if (file.exists("../data/results/p_ag_spec.rds")) {
  obj_name <- load("../data/results/p_ag_spec.rds")
  p_ag_spec <- get(obj_name)
}
if (file.exists("../data/results/density_supp_TSNE.rds")) {
  obj_name <- load("../data/results/density_supp_TSNE.rds")
  suppl_DENS <- get(obj_name)
}
if (file.exists("../data/results/TZ_supplementary_layers.rds")) {
  obj_name <- load("../data/results/TZ_supplementary_layers.rds")
  suppl_layer <- get(obj_name)
}
...
red = 0.5
axis_size = axis_size1 - red
ticks_size = ticks_size1 - red
legend_size = legend_size1 - red
facet_text = facet_text1 - red

# Apply themes to Vicinity plots
v1 <- v1 + labs(x = "Neighborhood size") + theme(
  ...
  axis.text.y = element_text(size = ticks_size),
  legend.title = element_text(size = legend_size),
  legend.text = element_text(size = legend_size),
  strip.text = element_text(size = facet_text),
  text = element_text(family = "DejaVu Sans")
)
if (length(v1$layers) >= 5) v1$layers[[5]]$geom_params$force_pull <- 0.5

s1 <- s1 + labs(x = "Neighborhood size") + theme(
  legend.position = "none",
  plot.title = element_blank(),
  axis.title.x = element_text(size = axis_size),
  axis.title.y = element_text(size = axis_size),
...
if (length(s1$layers) >= 5) s1$layers[[5]]$geom_params$force_pull <- -0.015

dms1 <- dms1 + labs(x = "Neighborhood size") + theme(
  legend.position = "none",
  plot.title = element_blank(),
  axis.title.x = element_text(size = axis_size),
  axis.title.y = element_text(size = axis_size),
  ...
  labeller = as_labeller(c(
    "antiberta2-cssp" = "AB2",
    esm2 = "ESM2",
    brian_hie = "Influenza HA\nShanker",
    porebski = "HER2\nPorebski",
    alphaseq = "SARS-CoV-2 HR2\nEngelhart",
    covabdab = "SARS-CoV-2 RBD\nCov-AbDab"
  ))
)
if (length(dms1$layers) >= 5) dms1$layers[[5]]$geom_params$force_pull <- -0.015

# Saving figures
dir.create("../figures/airr_atlas", showWarnings = FALSE, recursive = TRUE)

if (exists("density_main_fig")) {
  density_main_fig <- density_main_fig + theme(
    legend.position = "none",
    plot.title = element_blank(),
    axis.title.x = element_text(size = axis_size),
    axis.title.y = element_text(size = axis_size),
  ...
  ggsave("../../fig_3_antigen_specificity/output/fig_3c_new_density_diff.png", density_main_fig, width = 9, height = 6, dpi = 300)
}

ggsave("../output/fig_4c_vicinity_density_prelim.png", v1, width = 8, height = 4.5, dpi = 300)
ggsave("../output/fig_4d_vicinity_sample_size_v2.png", s1, width = 8, height = 4.5, dpi = 300)
ggsave("../output/fig_4e_vicinity_density_dms_v2.png", dms1, width = 8, height = 4.5, dpi = 300)

if (exists("p_ag_spec")) {
  p_ag_spec <- p_ag_spec + theme(
    plot.title = element_blank(),
    axis.title.x = element_text(size = axis_size),
    axis.title.y = element_text(size = axis_size),
    axis.text.x = element_text(size = ticks_size),
  ...
  ggsave("../../fig_3_antigen_specificity/output/fig_3b_ag_density.png", p_ag_spec, width = 16, height = 4.5, dpi = 301)
}

if (exists("suppl_DENS")) {
  suppl_DENS <- suppl_DENS + theme(
    plot.title = element_blank(),
    axis.title.x = element_text(size = axis_size),
    axis.title.y = element_text(size = axis_size),
    axis.text.x = element_text(size = ticks_size),
  ...
  ggsave("../../supplementary/fig_s7/output/fig_s7_supplementary_density_tsne.png", suppl_DENS, width = 12, height = 4.5, dpi = 600)
}
```
