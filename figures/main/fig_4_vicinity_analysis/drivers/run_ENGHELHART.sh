...
input_metadata="../data/sequences/alphaseq_sars/metadata_alphaseq_HB_LB.csv"
LD_matrix='../LD_alphaseq_HB_LB.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    input_idx="/scratch/niccoloc/alphaseq44/${model}/alphaseq_paired_chain_${model}_idx.csv"
    input_embeddings="/scratch/niccoloc/alphaseq44/${model}/${complexity}/alphaseq_${model}_${complexity}_layer_$((layer+1)).npy"
  else
      ...
      for complexity in "${complexities[@]}"; do
        ...
        #   fi
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../Vicinity_results_ALPHASEQ_Density2_HB_LB"
          else
            result_dir="../Vicinity_results_ALPHASEQ_Density2_sample${sample_size}"
          fi
