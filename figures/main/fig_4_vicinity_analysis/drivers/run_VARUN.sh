...
input_metadata="../data/sequences/cr9114_hie_metadata.csv"
LD_matrix='../LD_mat_brian_hie.npy'
...
get_paths() {
  ...
  if [[ "$chain" == "paired_chain" || "$chain" == "all_cdrh" ]]; then
    ...
    input_idx="/doctorai/userdata/airr_atlas/data/embeddings/brian_hie/${model}/brian_hie${chain}_${model}_idx.csv"
    input_embeddings="/doctorai/userdata/airr_atlas/data/embeddings/brian_hie/${model}/${complexity}/brian_hie_${model}_${model}_${complexity}_layer_$((layer+1)).npy"
  else
      ...
      for complexity in "${complexities[@]}"; do
        for chain in "${chains[@]}"; do
          ...
          if [[ $sample_size == 0 ]]; then
            result_dir="../Vicinity_results_BRIANHIE_Density2"
          else
            result_dir="../Vicinity_results_BRIANHIE_Density2_sample${sample_size}"
          fi
