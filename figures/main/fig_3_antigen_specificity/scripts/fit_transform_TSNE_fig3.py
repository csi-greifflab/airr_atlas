...
# idx=pd.read_csv('../data/embeddings/ireceptor_NEW_final_idx.csv')
...
,
    ...
    out_path = f"../data/results/tsne_Ireceptor_{emb_name}.csv"
    #save the fitted tsne model 
    try:
        joblib.dump(tsne_embeddings, f"../data/results/tsne_model_Ireceptor_{emb_name}_NEW.joblib", compress=3)
...
# paths for tz dataset and embeddings
input_metadata="../data/metadata/tz_heavy_chains_airr_dedup_final.tsv"
esm2_tz='../data/embeddings/tz_cdr3_100k_esm2.npy'
ab2_tz='../data/embeddings/tz_cdr3_100k_ab2.npy'
idx_reference='../data/embeddings/tz_cdr3_100k_idx.csv'


metadata_porebski='../data/metadata/porebski_metadata_vsALL.csv'
esm2_pb='../data/embeddings/porebski_cdr3_only_esm2.npy'
ab2_pb='../data/embeddings/porebski_cdr3_only_ab2.npy'
idx_reference_pb='../data/embeddings/porebski_cdr3_only_idx.csv'
...
for emb_name, tsne_model in tsne_models.items():
    ...
    out_path = f"../data/results/tsne_tz_{emb_name}_NEW.csv"
...
for emb_name, tsne_model in tsne_models.items():
    ...
    # Save the transformed dataframe
    out_path = f"../data/results/tsne_porebski_{emb_name}_NEW.csv"
...
import joblib
tsne_models={}
tsne_models['esm2'] = joblib.load(f"../data/results/tsne_model_Ireceptor_esm2_NEW.joblib")
tsne_models['ab2']  = joblib.load(f"../data/results/tsne_model_Ireceptor_ab2_NEW.joblib")
...
for emb_name, tsne_model in tsne_models.items():
    ...
    # Save the transformed dataframe
    out_path = f"../data/results/tsne_ireceptor_{emb_name}_PGEN.csv"
...
#load syhtetic prepost data
prepost = pd.read_csv('../data/metadata/post_selection_1M_pgen_NEW.csv')
# Embeddings paths would be updated similarly
...
for emb_name, tsne_model in tsne_models.items():
    ...
    # Save the transformed dataframe
    out_path = f"../data/results/tsne_POST_1M_{emb_name}_NEW.csv"
    transformed_prepost.to_csv(out_path, index=False)
    print(f"Saved transformed TSNE dataframe for prepost {emb_name} to {out_path}")
...
ireceptor_metadata = '../data/metadata/ireceptor_3M_nodup_max35L_pgen.csv'
# ireceptor_embeddings_tensor = torch.load('../data/embeddings/ireceptor_3M_esm2.pt').float()
...
for epoch in trange(epochs, desc="Training Epochs"):
    ...
    with open(f"../data/results/train_loss_PGEN_regressor.txt", "w") as f:
...
#save the model
joblib.dump(tsne_embeddings, f"../data/results/tsne_model_Ireceptor_{emb_name}.joblib")
