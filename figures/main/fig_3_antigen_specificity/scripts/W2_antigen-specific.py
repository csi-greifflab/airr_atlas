...
# import os
# os.chdir('./')
from figures.main.fig_3_antigen_specificity.scripts.get_W2 import get_w2
...
# DATA PATHS
ireceptor_metadata = "../data/metadata/ireceptor_NEW_final_onlyseqid.csv"
irecep_esm2 = "../data/embeddings/ireceptor_2M_esm2_mean_pooled.npy"
irecep_ab2 = "../data/embeddings/ireceptor_2M_ab2_mean_pooled.npy"
ag_metadata = "../data/metadata/antigen_specific_df_2025.csv"
ag_esm2 = "../data/embeddings/ag_dataset_esm2_embeddings.npy"
ag_ab2 = "../data/embeddings/ag_dataset_ab2_embeddings.npy"
postselection = "../data/embeddings/postselection_1M_esm2_mean_pooled.npy"

ag_OHE = "../data/embeddings/w2_AG_DATA_OHE.pt"
irecep_OHE = "../data/embeddings/w2_irecep_OHE.pt"
...
def load_data(
    ...
    print(
        f"Number of sequences in the dataset after de-deuplication: {len(df)}, per class: {df[df_affinity_colname].value_counts()}"
    )
...
ag_idx = pd.read_csv("../data/embeddings/antigen_specific_2025_idx.csv")
...
ireceptor_metadata = "../data/metadata/ireceptor_NEW_final_onlyseqid.csv"
ag_metadata = "../data/metadata/antigen_specific_df_2025.csv"
ag_idx_path = "../data/embeddings/antigen_specific_2025_idx.csv"
post_meta = "../data/metadata/post_selection_1M_pgen_NEW.csv"
...
MODEL_PATHS = {
    "esm2": {
        "ag": "../data/embeddings/ag_dataset_esm2_embeddings.npy",
        "irec": "../data/embeddings/ireceptor_2M_esm2_mean_pooled.npy",
        "post": "../data/embeddings/postselection_1M_esm2_mean_pooled.npy",
    },
    "ab2": {
        "ag": "../data/embeddings/ag_dataset_ab2_embeddings.npy",
        "irec": "../data/embeddings/ireceptor_2M_ab2_mean_pooled.npy",
        "post": "../data/embeddings/postselection_1M_ab2_mean_pooled.npy",
    },
    "ohe": {
        "ag": "../data/embeddings/w2_AG_DATA_OHE.pt",
        "irec": "../data/embeddings/w2_irecep_OHE.pt",
        "post": "../data/embeddings/w2_POSTSEL_OHE.pt",
    },
}
...
ireceptor_metadata_df_dedup = ireceptor_metadata_df.drop_duplicates(
    subset=["junction_aa"]
)
...
for name1, name2 in itertools.combinations(names, 2):
    ...
    #     continue
    print(
        f"Computing W2 distance between {name1} and {name2}, shapes: {ag_embeddings_dict_X[name1].shape} vs {ag_embeddings_dict_X[name2].shape}"
    )
...
def split_and_compute_w2(ag, ag_embeddings, repeats=10, seed=42, batch_size=None):
    ...
    for repeat in range(repeats):
        ...
        # print ag size
        print(
            f"Repeat {repeat+1}: ag1 size = {ag1.shape[0]}, ag2 size = {ag2.shape[0]}"
        )
        name1_ag = f"{ag}_1"
        name2_ag = f"{ag}_2"
        res = compute_W2_pairwise(
            ag1, ag2, x1_name=name1_ag, x2_name=name2_ag, device="cpu"
        )
...
for batch_size in batch_sizes:
    ...
    results = Parallel(n_jobs=80)(
        delayed(split_and_compute_w2)(
            ag, ag_embeddings_dict_X[ag], 10, 42, batch_size=batch_size
        )
        for ag in names
    )
    for ag, result in results:
        ...
        print(
            f"{ag} (batch {batch_size}): mean W2={result[0]:.4f}, std={result[1]:.4f} over 10 splits"
        )
...
mean_std_per_batch = {
    batch: {
        ag: (vals[0], vals[1])
        for (ag, b), vals in ag_repeat_tests.items()
        if b == batch
    }
...
df_ag_repeat = pd.DataFrame.from_dict(
    ag_repeat_tests, orient="index", columns=["mean_w2", "std_w2", "all_w2s"]
)
...
df_ag_repeat_all = pd.DataFrame(
    df_ag_repeat["all_w2s"].tolist(),
    columns=[f"repeat_{i+1}" for i in range(len(df_ag_repeat["all_w2s"].iloc[0]))],
    index=df_ag_repeat.index,
).T
...
new_names = [
    f"{new_names_id[i]}\n(n={antigen_counts[names[i]]})"
    for i in range(len(new_names_id))
]
...
plt.xticks(
    rotation=45, ha="right", fontsize=11, rotation_mode="anchor", position=(0, 0)
)
