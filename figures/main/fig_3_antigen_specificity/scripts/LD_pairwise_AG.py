...
post_df = pd.read_csv("../data/metadata/post_selection_1M_pgen_NEW.csv")
...
ireceptor_metadata_df = ireceptor_metadata_df.sample(
...
for i, spec1 in enumerate(tqdm(spec_names, desc="Specificities", unit="spec")):
    ...
    for j, spec2 in enumerate(spec_names):
        ...
        result = {"matrix": mat, "mean": mean_dist, "std": std_dist, "shape": mat.shape}
...
intra_mean = np.mean(
    [v["mean"] for v in intra_results.values() if v["mean"] is not None]
)
inter_mean = np.mean(
    [v["mean"] for v in inter_results.values() if v["mean"] is not None]
)
...
plt.tight_layout()
plt.show()
plt.savefig("../data/results/mean_levenshtein_distance_heatmap_LEV_NORM.png")
...
with open(
    "../data/results/pairwise_levenshtein_distance_results_LEV_NORM.pkl", "wb"
) as f:
...
with open(
    "../data/results/pairwise_levenshtein_distance_results_LEV_NORM.pkl", "rb"
) as f:
