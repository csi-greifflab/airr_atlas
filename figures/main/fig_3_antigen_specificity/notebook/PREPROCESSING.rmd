...
```{r}
library("withr", lib.loc= "../libR2")
library(dplyr , lib.loc = "../libR2")
library(tidyr , lib.loc = "../libR2")
library(ggplot2 , lib.loc = "../libR2")
library(viridis , lib.loc = "../libR2")
library(stringr , lib.loc = "../libR2")
library(patchwork , lib.loc = "../libR2")
library(data.table , lib.loc = "../libR2")
# library(Peptides, lib.loc= "../libR")
library(foreach, lib.loc= "../libR2")
# library(scico, lib.loc= "../libR2")
# library(BiocManager , lib.loc= "../libR2")
library(ggseqlogo , lib.loc= "../libR2")
# library(seqinr , lib.loc= "../libR2")
library("labeling", lib.loc= "../libR2")
library("isoband", lib.loc= "../libR2")
# library("ggridges", lib.loc= "../libR2")
library("withr", lib.loc= "../libR2")
 
 # install.packages("ggVennDiagram", lib="../libR2")
 # install.packages("ggridges", lib="../libR2")
# install.packages("doPar", lib="../libR2")
# install.packages("seqinr", lib="../libR2")
...
ggsave("../../supplementary/fig_s15/output/fig_s15_alphaseq_scatter_hb_selection.png", width = 10, height = 7, units = "in", dpi = 300)
```
...
ggsave(
  filename = "../../supplementary/fig_s14/output/fig_s14_cr9114.png",
  plot = last_plot(),
  width = 10,
  height = 7,
  units = "in",
...
vh_mixcr = fread('../mixcr_output/VH_aligned.tsv') %>% 
    mutate(VH =str_remove(aaSeqVDJRegion,"_$")) %>% 
  select(sequence_id = descrsR1,
         VH,
         HCDR3 =  aaSeqCDR3)  
vl_mixcr = fread('../mixcr_output/VL_aligned.tsv')  %>% 
    mutate(VL =str_remove(aaSeqVDJRegion,"_$")) %>% 
  select(sequence_id = descrsR1,
         VL  ,
         LCDR3 =  aaSeqCDR3)  
 
bg_VH= fread('../mixcr_output/VH_aligned_airr.tsv')  %>% 
  select(-sequence_id) 
 
bg_VL= fread('../mixcr_output/VL_aligned_airr.tsv') %>% 
  select(-sequence_id)
...
db_spec = fread("../merged_df_may_2024_preprocessed_V2.csv")
table(db_spec$dataset,db_spec$specificity)
x3 = db_spec %>% 
  mutate(junction_length = str_length(junction_aa),
...
fwrite(x3,"../data/sequences/antigen_specific_df_2025.csv")
#write fasta

seqinr::write.fasta(as.list(x3$sequence_aa),
                    names = x3$sequence_id, 
                    file.out = "../data/sequences/antigen_specific_2025.fasta")


x1= fread("../data/sequences/antigen_specific_df_2025.csv")
...
pre<-fread("../data/sequences/preselection.csv"  ) 
pre<-pre %>% mutate(sequence_id = str_c("pre_",1:nrow(.)))
post<-fread("../data/sequences/postselection.csv"   )
...
fwrite(sonia_2M, "../prepost_sonia_2M.csv")

#write fasta
seqinr::write.fasta(as.list(sonia_2M$amino_acid),
                    names = sonia_2M$sequence_id, 
                    file.out = "../prepost_sonia_2M.fasta")


# reload with the pgen calculated
sonia_2M=fread('../prepost_sonia_2M_pgens_OK.csv') %>% distinct()
sonia_2M %>% fwrite("../prepost_sonia_2M_pgens_OK2.csv")
...
seqinr::write.fasta(as.list(new_ireceptor_2$junction_aa),
                    ...
                    file.out = "../ireceptor_NEW_final.fasta")

#input for tsne mapping
new_ireceptor_2 %>% 
  fwrite("../ireceptor_NEW_final_onlyseqid.csv")
...
new_ireceptor_sonia %>% 
  ...
  # slice_head(n=10000) %>% 
  fwrite("../ireceptor_NEW_sonia_INPUT.csv")
...
ireceptor_xy= fread('../vae_umap_dfs/tsne_Ireceptor_ab2.csv')   
ireceptor_xy %>%
  rename(pgen_old=pgen) %>%
  left_join(new_df %>% select(-junction_aa), by="sequence_id") %>% distinct() %>%   
  fwrite("../vae_umap_dfs/ireceptor_NEW_final_xy_ab2.csv")

ireceptor_xy= fread('../vae_umap_dfs/tsne_Ireceptor_esm2.csv')   
ireceptor_xy %>% 
  rename(pgen_old=pgen) %>%
  left_join(new_df %>% select(-junction_aa), by="sequence_id")%>% distinct() %>%  
  fwrite("../vae_umap_dfs/ireceptor_NEW_final_xy_esm2.csv")
