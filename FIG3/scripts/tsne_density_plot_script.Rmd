...
# LOAD SUMMARIZED DATA FOR GITHUB REPRODUCIBILITY
# The original scripts loaded 1M+ rows; here we use the provided summarized rds/csv.
if (file.exists("../data/results/p_ag_spec.rds")) {
  load("../data/results/p_ag_spec.rds")
}

tsne_df_tz=bind_rows(
  fread("../data/results/tsne_tz_esm2_NEW.csv") %>% # Assumed to be in results or unzipped
    mutate(model = 'esm2',specificity=binding_label),
  fread("../data/results/tsne_tz_ab2_NEW.csv") %>%
    mutate(model = 'ab2',specificity=binding_label),
  fread("../data/results/tsne_porebski_ab2_NEW.csv") %>%
    mutate(model = 'ab2', label= 'porbeski',specificity=binding_label) %>% filter(binding_label =='hb'),
  fread("../data/results/tsne_porebski_esm2_NEW.csv") %>%
...
save(p_ag_spec , file = "../data/results/p_ag_spec.rds")
...
save(final_plot, file = "../data/results/density_DIFF_plot.rds")
...
ggsave("../figures/FIG_3C_New_Density_DIFF.png", plot = final_plot,width =10, height =6.25, units = "in", dpi = 300)
...
save(supp_dens, file = "../data/results/density_supp_TSNE.rds")
